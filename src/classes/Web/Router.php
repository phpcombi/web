...
class Router
{
    ...
    public function addPsr4(string $base_dir, ...$namespaces): self {
        $this->psr4data[$base_dir] = $namespaces;
        return $this;
    }
    ...
    protected function makeDispatcher() {
        $dispatcher = \FastRoute\cachedDispatcher(function(FastRouter $r) {
            ...
            'cacheFile'     => core::path('tmp', 'route.cache'),
            'cacheDisabled' => !core::isProd(),
        ]);
    ...
    protected function applyByPsr4(array $data) {
        foreach ($data as $base_dir => $namespaces) {
            foreach ($namespaces as $namespace) {
                $path = $base_dir.strtr($namespace, '\\', DIRECTORY_SEPARATOR);
    ...
    protected function applyByDir(array $data) {
        foreach ($data as $dir) {
            $directory = new core\Resource\Directory($dir);
            foreach ($directory->traversal() as $fileinfo) {
                if ($fileinfo->isDir()) {
                    continue;
                }
